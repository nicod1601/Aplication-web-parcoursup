...
class CandidatRepository
{
    ...
    public function getAllCandidats(?int $anneeDeb = null, ?int $anneeFin = null)
    {
        $anneeLabel = ($anneeDeb !== null && $anneeFin !== null)
            ? sprintf('%d/%d', $anneeDeb, $anneeFin)
            : '2024/2025';
        ...
        $sql = "SELECT
            ...
            fi.libFiliere                               AS \"Filiere (pour scolarité du supérieur)- Libellé {$anneeLabel}\",
            fo.libFormation                             AS \"Formation - Libellé (Saisie manuelle) {$anneeLabel}\",
            spe.libSpe                                  AS \"Spécialité / Mention - Libellé {$anneeLabel}\",

            e.nomEtab                                   AS \"Nom Etablissement origine {$anneeLabel}\",
            l.nomCommu                                  AS \"Commune Etablissement origine - Libellé {$anneeLabel}\",
            e.codePost                                  AS \"Commune Etablissement origine - CodePostal {$anneeLabel}\",
            l.nomDept                                   AS \"Département Etablissement origine - Libellé {$anneeLabel}\",
            l.pays                                      AS \"Pays Etablissement origine - Libellé {$anneeLabel}\",
        ...
        $params = [];
        if ($anneeDeb !== null && $anneeFin !== null) {
            $sql .= " WHERE c.anneeDeb = ? AND c.anneeFin = ?";
            $params[] = $anneeDeb;
            $params[] = $anneeFin;
        }
